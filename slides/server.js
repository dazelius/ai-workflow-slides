...
- rename_slide/duplicate_slide/move_slide/set_theme는 미리보기 없이 즉시 적용됩니다. set_theme은 발표 전체의 분위기를 바꾸므로 사용자가 명확히 테마 변경을 원할 때만 쓰세요.
...
const AGENT_TOOLS = [
  ...
  {
    name: "rename_slide",
    description: "슬라이드의 사이드바 제목을 바꾼다(슬라이드 본문 내용은 건드리지 않음).",
    input_schema: {
      type: "object",
      properties: {
        index: { type: "integer", description: "제목을 바꿀 슬라이드의 0부터 시작하는 인덱스" },
  ...
  },
  {
    ...
    description: "슬라이드 한 장을 그대로 복제해서 원본 바로 뒤에 넣는다. 비슷한 레이아웃의 슬라이드를 여러 장 만들 때 create_slide보다 빠르고 안전하다.",
    input_schema: {
      type: "object",
      properties: { index: { type: "integer", description: "복제할 슬라이드의 0부터 시작하는 인덱스" } },
      required: ["index"],
    },
  },
  {
    name: "set_theme",
    description: "발표 전체의 테마(슬라이드 마스터: 배경/텍스트/포인트 색)를 프리셋으로 바꾼다. 모든 슬라이드에 즉시 적용된다.",
    input_schema: {
      type: "object",
      properties: {
        preset: { type: "string", description: "테마 프리셋 키. dark(다크 기본) / light(화이트) / navy(네이비) / warm(웜 베이지) / mono(모노 그레이) 중 하나" },
      },
      required: ["preset"],
    },
  },
...
// 클립보드/AI로 들어온 이미지를 base64로 HTML 안에 박아두면 슬라이드 파일이
// 수 MB까지 커진다(로드 느려짐 + AI가 읽고 쓸 때마다 거대 문자열 통과).
// 그래서 이미지는 항상 실제 파일(assets/img/)로 저장하고 HTML에는 경로만 남긴다.
// 같은 내용이면 같은 해시 파일명이 되므로 중복 저장도 자동으로 방지된다.
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "POST" && url === "/api/save-image") {
    ...
    req.on("end", () => {
      try {
        const { mime, b64 } = JSON.parse(ibody);
        const ext = IMAGE_MIME_EXT[mime];
        if (!ext || typeof b64 !== "string" || !b64) {
          return send(res, 400, JSON.stringify({ ok: false, error: "invalid image" }), {
            "Content-Type": "application/json; charset=utf-8",
          });
        }
        const buf = Buffer.from(b64, "base64");
        if (!buf.length || buf.length > 25 * 1024 * 1024) {
          return send(res, 400, JSON.stringify({ ok: false, error: "invalid image size" }), {
            "Content-Type": "application/json; charset=utf-8",
          });
        }
        ...
        console.log(`이미지 파일로 저장됨: ${rel} (${(buf.length / 1024).toFixed(0)}KB)`);
        send(res, 200, JSON.stringify({ ok: true, url: rel }), {
          "Content-Type": "application/json; charset=utf-8",
        });
      } catch (e) {
        send(res, 500, JSON.stringify({ ok: false, error: String(e) }), {
